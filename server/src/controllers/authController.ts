...
export const forgotPassword = async (req: Request, res: Response) => {
  try {
    ...
    const user = await User.findOne({ email }).select(
      "+resetPasswordAttempts +resetPasswordLastAttempt",
    );
    if (!user) {
      return res.json({
        success: true,
    ...
    if (
      ...
      user.resetPasswordLastAttempt > fifteenMinutesAgo
    ) {
      if ((user.resetPasswordAttempts || 0) >= 3) {
        return res.status(429).json({
          ...
          retryAfter: Math.ceil(
            (user.resetPasswordLastAttempt.getTime() +
              15 * 60 * 1000 -
              now.getTime()) /
              1000,
          ),
    ...
    if (!emailSent) {
      ...
      return res.json({
        success: true,
        message:
          "Email service unavailable. Please use the verification code below or contact support.",
